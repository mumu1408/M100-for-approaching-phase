...
ros::Subscriber velocity_control_z_sub;
...
double velocity_control_effort_z;
...
void velocityControlEffortZCallback(std_msgs::Float64 velocity_control_effort_z_msg)
{
	velocity_control_effort_z = velocity_control_effort_z_msg.data;
}
...
int main(int argc, char **argv)
{
	...
	velocity_control_z_sub = nh.subscribe("/m100_track/velocity_control_effort_z", 10, velocityControlEffortZCallback);
	...
	while(ros::ok())
	{
		...
		if(position_track_enabled)
		{
			...
			else
			{
				...
				velocity_control.request.vx = velocity_control_effort_x;
				velocity_control.request.vy = velocity_control_effort_y;
				velocity_control.request.vz = velocity_control_effort_z;
				velocity_control.request.yawRate = velocity_control_effort_yaw;
