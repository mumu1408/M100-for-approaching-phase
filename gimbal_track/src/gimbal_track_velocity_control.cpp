...
{
	ros::init(argc, argv, "gimbal_track_velocity_control_node");
	ros::NodeHandle nh;

	gimbal_control_pitch_sub = nh.subscribe("/gimbal_track/gimbal_control_effort_pitch", 10, gimbalPitchControlEffortXCallback);
	gimbal_control_yaw_sub = nh.subscribe("/gimbal_track/gimbal_control_effort_yaw", 10, gimbalYawControlEffortYCallback);
	gimbal_track_enable_sub = nh.subscribe("/gimbal_track/gimbal_track_enable", 1, gimbal_track_enable_callback );
